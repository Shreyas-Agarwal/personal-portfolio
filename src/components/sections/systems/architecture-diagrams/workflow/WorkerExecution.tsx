...
import { Database, MessageSquare, RadioTower, Workflow } from "lucide-react";
import { DistributionNode, MiniService, WorkerTask } from "./SharedComponents";
    ...
    >
      <div className="mb-10 flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
        ...
        <p className="max-w-md text-sm leading-relaxed text-white/40">
          Worker execution was intentionally separated from product servers, allowing asynchronous
          operational tasks to scale independently from runtime application clusters.
        </p>
      </div>
      ...
      <div className="grid gap-8 xl:grid-cols-[1fr_3fr_1fr]">
        ...
        <div className="space-y-5">
          <MiniService icon={<Workflow size={16} />} title="Workflow Triggers" />

          <MiniService icon={<MessageSquare size={16} />} title="Notification Requests" />

          <MiniService icon={<Database size={16} />} title="Processing Outputs" />
        </div>
        ...
        <div className="rounded-3xl border border-white/[0.08] bg-black/30 p-8">
          <div className="flex items-center gap-4">
            <div className="rounded-2xl border border-emerald-500/15 bg-emerald-500/[0.05] p-3">
              <RadioTower className="text-emerald-300/70" size={22} />
            </div>

            <div>
              <h4 className="text-xl font-medium text-white/90">Queue-Driven Worker Activation</h4>
          ...
          <div className="mt-8 rounded-2xl border border-white/[0.06] bg-white/[0.02] p-5">
            <p className="text-sm leading-relaxed text-white/45">
              Product clusters delegated asynchronous workloads into queue-driven execution
              infrastructure, reducing server coupling and allowing independent operational
              scalability.
            </p>
          </div>
        </div>
        ...
        {/* LAMBDA EXECUTION */}
        <div className="space-y-5">
          <DistributionNode title="Lambda Workers" subtitle="Elastic serverless execution" />
