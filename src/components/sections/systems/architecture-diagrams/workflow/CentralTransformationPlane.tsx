...
import { DistributionNode, MiniService, PipelineCapability, PlaneCard } from "./SharedComponents";
    ...
    >
      <div className="mb-10 flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
        ...
        <p className="max-w-md text-sm leading-relaxed text-white/40">
          Instead of duplicating transformation logic across product deployments, operational data
          pipelines were centralized into containerized scheduled processing infrastructure.
        </p>
      </div>
      ...
      <div className="grid gap-8 xl:grid-cols-[1fr_4fr_1fr] xl:items-stretch">
        ...
        <div className="space-y-5">
          <MiniService icon={<Database size={16} />} title="Raw Operational Data" />

          <MiniService icon={<Layers3 size={16} />} title="Cross-Product Inputs" />
        </div>
        ...
        <div className="relative overflow-hidden rounded-[36px] border border-cyan-500/15 bg-black/30 p-10">
          ...
          <div className="mt-8 rounded-2xl border border-white/[0.06] bg-white/[0.02] p-5">
            <p className="text-sm leading-relaxed text-white/45">
              Product environments no longer owned raw transformation workloads independently.
              Instead, they consumed processed, operationally-scoped outputs from a centralized
