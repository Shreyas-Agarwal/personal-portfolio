...
export function ArchitectureNode({
  ...
  return (
    ...
    >
      ...
      <p className="mt-2 text-xs leading-relaxed text-white/40">{subtitle}</p>
    </motion.div>
  );
}

export function MiniService({ icon, title }: { icon: React.ReactNode; title: string }) {
  return (
...
export function DistributionNode({ title, subtitle }: { title: string; subtitle: string }) {
  return (
    <motion.div
...
export function ServiceRow({ icon, label }: { icon: React.ReactNode; label: string }) {
  return (
    <motion.div
      whileHover={{ x: 4 }}
  ...
  return (
    <motion.div
      ...
      className={`rounded-2xl border p-4 transition-all ${
        highlight ? "border-cyan-500/30 bg-cyan-500/[0.08]" : "border-white/[0.06] bg-white/[0.02]"
      }`}
    >
      <div className="flex items-center justify-between">
        <span
          className={`text-[10px] font-bold uppercase tracking-tighter ${
            highlight ? "text-cyan-400" : "text-white/40"
          }`}
        >
          {tier}
        </span>
        <div
          className={`h-1.5 w-1.5 rounded-full ${
            highlight ? "bg-cyan-400 animate-pulse" : "bg-white/20"
          }`}
        />
      </div>
...
export function FederatedCard({ title, description }: { title: string; description: string }) {
  return (
    <motion.div
      whileHover={{ y: -2 }}
...
export function ArchitectureConnectorVertical({ top }: { top: string }) {
  return (
    <div
...
export function PlaneCard({ title, description }: { title: string; description: string }) {
  return (
    <motion.div
      whileHover={{ y: -2 }}
      className="rounded-3xl border border-white/[0.06] bg-black/20 p-6"
...
export function PipelineCapability({ title, detail }: { title: string; detail: string }) {
  return (
...
export function WorkerTask({ label }: { label: string }) {
  return (
    <motion.div
      whileHover={{ x: 3 }}
